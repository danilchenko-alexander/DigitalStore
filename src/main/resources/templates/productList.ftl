...
<div class="col-sm-4">
    ...
    <#if catalog == "Computer">
        <#list computers?if_exists as computer>
            <div>
                <b>${computer.name}</b>
                <span>${computer.price?replace(",",".")}</span>
                <strong>${computer.description}</strong><br>
                <div>
                    <#if computer.filename??>
                        <img src="/img/${computer.filename}">
                    </#if>
                </div>
                <a href="/catalogList/${catalog}/edit/${subcatalog.id}/editComputer/${computer.id}">edit</a>
            </div>
        <#else>
            No products
        </#list>
    <#elseif catalog=="Phone">
        <#list phones?if_exists as phone>
            <div>
                ...
                <a href="/catalogList/${catalog}/edit/${subcatalog.id}/editComputer/${phone.id}">edit</a>
            </div>
        <#else>
            No products
        </#list>
